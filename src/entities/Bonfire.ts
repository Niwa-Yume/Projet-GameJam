...
export function createBonfire(scene: Phaser.Scene, x: number, y: number): void {
  // Créer la fonction de fallback procédural
  const createProceduralBonfire = () => {
  ...
  // Utiliser le fallback procédural qui est plus stable et fiable
  createProceduralBonfire();
}
