...
// Import des Classes Logiques (Entités)
import { Tower } from '../entities/Tower';
import { Wall } from '../entities/Wall';
import { Campfire } from '../entities/Campfire';
import { Generator } from '../entities/Generator';
import { Forge } from '../entities/Forge';
import { Storage as StorageBuilding } from '../entities/Storage'; // ⚠️ ALIAS ICI
...
export class BuildingFactory {
    ...
    public createBuilding(kind: BuildingKind, x: number, y: number, buildingManager: any): Phaser.GameObjects.Container {
        switch (kind) {
            ...
            default: throw new Error(`Type de bâtiment inconnu: ${kind}`);
        }
    }

    private createTower(x: number, y: number, buildingManager: any): Phaser.GameObjects.Container {
        // 1. INSTANCIATION LOGIQUE
        const logic = new Tower({
            type: 'tower',
            x,
            y,
            baseCost: GameConstants.INITIAL_TOWER_COST,
            baseHp: 100
        });

        // 2. CRÉATION VISUELLE (Dark Souls)
        const towerContainer = this.scene.add.container(x, y).setDepth(10);
        this.scene.physics.add.existing(towerContainer, true);
        ...
        // 3. LIAISON
        logic.sprite = towerContainer;
        towerContainer.setData('buildingInstance', logic);
        towerContainer.setData({ fireTimer, glow });

        const health = new HealthComponent(towerContainer, 100);
        health.on('die', () => logic.destroy());
        ...
        tower.on('pointerover', () => {
            rangeGfx.clear().lineStyle(1, 0x6b8fa5, 0.85).strokeCircle(x, y, logic.getStats().level * 10 + 150).setVisible(true);
        });
        tower.on('pointerout', () => rangeGfx.setVisible(false));
        tower.on('pointerdown', (p: Phaser.Input.Pointer) => {
            if (p.rightButtonDown()) buildingManager.showUpgradeMenu(towerContainer, 'tower');
        });
    ...
    private createWall(x: number, y: number, buildingManager: any): Phaser.GameObjects.Container {
        const logic = new Wall({
            type: 'wall',
            x,
            y,
            baseCost: GameConstants.INITIAL_WALL_COST,
            baseHp: 200
        });

        const wallContainer = this.scene.add.container(x, y).setDepth(9);
        this.scene.physics.add.existing(wallContainer, true);
        ...
        logic.sprite = wallContainer;
        wallContainer.setData('buildingInstance', logic);

        const health = new HealthComponent(wallContainer, 200);
        health.on('die', () => logic.destroy());
        attachHealthBar(this.scene, wallContainer);

        wall.on('pointerdown', (p: Phaser.Input.Pointer) => {
            if (p.rightButtonDown()) buildingManager.showUpgradeMenu(wallContainer, 'wall');
        });
    ...
    private createGenerator(x: number, y: number, buildingManager: any): Phaser.GameObjects.Container {
        const logic = new Generator({
            type: 'generator',
            x,
            y,
            baseCost: GameConstants.INITIAL_GENERATOR_COST,
            baseHp: 120
        });

        const genContainer = this.scene.add.container(x, y).setDepth(9);
        this.scene.physics.add.existing(genContainer, true);
        ...
        // Event pour l'affichage flottant
        const genTimer = this.scene.time.addEvent({
            delay: GameConstants.GENERATOR_TICK_MS,
            loop: true,
            callback: () => {
                const mul = logic.getProductionMultiplier ? logic.getProductionMultiplier() : 1;
        ...
        logic.sprite = genContainer;
        genContainer.setData('buildingInstance', logic);

        new HealthComponent(genContainer, 120);
        attachHealthBar(this.scene, genContainer);

        gen.on('pointerdown', (p: Phaser.Input.Pointer) => {
            if (p.rightButtonDown()) buildingManager.showUpgradeMenu(genContainer, 'generator');
        });
    ...
    private createCampfire(x: number, y: number, buildingManager: any): Phaser.GameObjects.Container {
        const logic = new Campfire({
            type: 'campfire',
            x,
            y,
            baseCost: GameConstants.INITIAL_CAMPFIRE_COST,
            baseHp: 100
        });
        ...
        graphics.fillStyle(0x000000, 0.5); graphics.fillEllipse(0, 15, 40, 15);
        graphics.fillStyle(0x202020, 1); graphics.fillCircle(0, 10, 20);

        graphics.fillStyle(0xdddddd, 1);
        graphics.save(); graphics.translateCanvas(0, 10); graphics.rotateCanvas(0.5); graphics.fillRect(-10, -2, 20, 4); graphics.restore();
        graphics.save(); graphics.translateCanvas(0, 10); graphics.rotateCanvas(-0.8); graphics.fillRect(-8, -2, 16, 3); graphics.restore();

        graphics.fillStyle(0x555555, 1);
        graphics.beginPath();
        graphics.moveTo(-2, -30); graphics.lineTo(3, -25); graphics.lineTo(1, -15); graphics.lineTo(4, -5); graphics.lineTo(2, 5); graphics.lineTo(-2, 5); graphics.lineTo(-3, -10); graphics.lineTo(-1, -20);
        graphics.closePath(); graphics.fillPath();
        graphics.lineStyle(2, 0x333333); graphics.moveTo(-8, -8); graphics.lineTo(8, -6); graphics.strokePath();
        ...
        const pulseTween = this.scene.tweens.add({
            ...
            scaleX: 1.1, scaleY: 1.2, alpha: 0.6, duration: 1000 + Math.random() * 500, yoyo: true, repeat: -1, ease: 'Sine.easeInOut'
        });

        const particleTween = this.scene.tweens.addCounter({
            from: 0, to: 100, duration: 3000, loop: -1,
            onUpdate: () => {
        ...
        logic.sprite = fireContainer;
        fireContainer.setData('buildingInstance', logic);
        ...
        hitArea.on('pointerdown', (p: Phaser.Input.Pointer) => {
            if (p.rightButtonDown()) buildingManager.showUpgradeMenu(fireContainer, 'campfire');
        });
    ...
    private createForge(x: number, y: number, buildingManager: any): Phaser.GameObjects.Container {
        const logic = new Forge({
            type: 'forge',
            x,
            y,
            baseCost: GameConstants.INITIAL_FORGE_COST,
            baseHp: 150
        });
        ...
        g.fillStyle(0x000000, 0.5); g.fillEllipse(0, 20, 40, 15);
        g.fillStyle(0x3e2723, 1); g.fillRect(-15, 0, 30, 20);
        g.lineStyle(1, 0x5d4037); g.strokeEllipse(0, 0, 15, 5);
        g.fillStyle(0x1a1a1a, 1); g.fillRect(-12, -5, 24, 5); g.fillRect(-8, -12, 16, 7); g.fillRect(-15, -18, 30, 8);
        g.beginPath(); g.moveTo(15, -18); g.lineTo(25, -18); g.lineTo(15, -12); g.fillPath();
        g.fillStyle(0x5d4037, 1); g.fillRect(8, 5, 4, 20);
        g.fillStyle(0x424242, 1); g.fillRect(5, 5, 10, 6);
        ...
        const glowTween = this.scene.tweens.add({
            targets: glow, alpha: { from: 0, to: 0.4 }, scale: { from: 1, to: 2 }, duration: 2000, yoyo: true, repeat: -1
        });

        logic.sprite = forgeContainer;
        forgeContainer.setData('buildingInstance', logic);

        new HealthComponent(forgeContainer, 150);
        attachHealthBar(this.scene, forgeContainer);

        hitArea.on('pointerdown', (p: Phaser.Input.Pointer) => {
            if (p.rightButtonDown()) buildingManager.showUpgradeMenu(forgeContainer, 'forge');
        });
    ...
    private createStorage(x: number, y: number, buildingManager: any): Phaser.GameObjects.Container {
        // ⚠️ UTILISE StorageBuilding au lieu de Storage
        const logic = new StorageBuilding({
            type: 'storage',
            x,
            y,
            baseCost: GameConstants.INITIAL_STORAGE_COST,
            baseHp: 140
        });
        ...
        shadow.fillStyle(0x000000, 0.4); shadow.fillEllipse(0, 12, 36, 12);

        const bodyGroup = this.scene.add.container(0, 0);
        const g = this.scene.add.graphics();
        g.fillStyle(0x2d1e16, 1); g.fillRect(-16, -16, 32, 24);
        g.lineStyle(2, 0x555555); g.strokeRect(-16, -16, 32, 24);
        g.fillStyle(0x3e2723, 1); g.beginPath(); g.moveTo(-18, -16); g.lineTo(18, -16); g.lineTo(16, -26); g.lineTo(-16, -26); g.closePath(); g.fillPath();
        g.fillStyle(0xdddddd, 1); g.fillTriangle(-10, -16, -6, -16, -8, -20); g.fillTriangle(6, -16, 10, -16, 8, -20);
        g.lineStyle(2, 0xaaaaaa); g.beginPath(); g.moveTo(18, -12); g.lineTo(30, -8); g.strokePath();

        bodyGroup.add(g);
        storageContainer.add([shadow, bodyGroup, hitArea]);

        const breatheTween = this.scene.tweens.add({
            targets: bodyGroup, scaleY: 1.05, scaleX: 1.02, y: -2, duration: 2500, yoyo: true, repeat: -1, ease: 'Sine.easeInOut'
        });

        logic.sprite = storageContainer;
        storageContainer.setData('buildingInstance', logic);
        storageContainer.setData('capInc', 50);

        new HealthComponent(storageContainer, 140);
        attachHealthBar(this.scene, storageContainer);

        hitArea.on('pointerdown', (p: Phaser.Input.Pointer) => {
            if (p.rightButtonDown()) buildingManager.showUpgradeMenu(storageContainer, 'storage');
        });
    ...
    private createBarracks(x: number, y: number, buildingManager: any): Phaser.GameObjects.Container {
        const logic = new Barracks({
            type: 'barracks',
            x,
            y,
            baseCost: GameConstants.INITIAL_BARRACKS_COST,
            baseHp: 150
        });
        ...
        g.fillStyle(0x000000, 0.5); g.fillEllipse(0, 20, 50, 20);
        g.fillStyle(0x4e342e, 1); g.fillRect(-20, -10, 40, 30);
        g.fillStyle(0x3e2723, 1); g.fillRect(-22, -10, 4, 30); g.fillRect(18, -10, 4, 30);
        g.fillStyle(0x263238, 1); g.beginPath(); g.moveTo(-28, -10); g.lineTo(0, -35); g.lineTo(28, -10); g.fillPath();
        g.fillStyle(0xffb300, 1); g.fillRect(-8, 0, 16, 12);
        g.fillStyle(0x5d4037, 1); g.fillRect(20, -5, 10, 2);
        g.fillStyle(0x8d6e63, 1); g.fillRect(24, -5, 8, 10);
        ...
        const lightTween = this.scene.tweens.add({
            targets: light, alpha: { from: 0.1, to: 0.3 }, duration: 1200, yoyo: true, repeat: -1, ease: 'Bounce.easeInOut'
        });

        logic.sprite = barracksContainer;
        barracksContainer.setData('buildingInstance', logic);

        new HealthComponent(barracksContainer, 150);
        attachHealthBar(this.scene, barracksContainer);

        hitArea.on('pointerdown', (p: Phaser.Input.Pointer) => {
            if (p.rightButtonDown()) buildingManager.showUpgradeMenu(barracksContainer, 'barracks');
        });
